...
export function TaskStatusControl({ task, tasks, userId, disabled = false, showSuccess = true }: { task: Task; tasks: Task[]; userId: string; disabled?: boolean; showSuccess?: boolean }) {
  ...
  return <div className="min-w-0 space-y-1">
    ...
    {status === 'waiting' && blockers.length > 0 && <p className="text-xs text-amber-700">前提の完了待ち：{blockers.map(blocker => blocker.task?.title || '前提タスクを確認できません').join('、')}</p>}
