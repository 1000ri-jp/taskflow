            ...
            return <section key={bundle.id} aria-label={bundle.title} className="border-b last:border-b-0">
              <div className="grid gap-3 px-4 py-3 lg:grid-cols-[220px_minmax(0,1fr)] lg:px-5">
                <div className="min-w-0 space-y-1">
                  ...
                  <p className="flex items-center gap-1.5 break-words text-[11px] text-muted-foreground">{destination.task && (destination.task.projectIcon || destination.task.projectIconUrl) && <ProjectIconBadge icon={destination.task.projectIcon} iconUrl={destination.task.projectIconUrl} color={destination.task.projectColor} priority={destination.task.priority} />}{candidateProjects.map(project => <ProjectIconBadge key={project.id} icon={project.icon} iconUrl={project.iconUrl} color={project.color} />)}{destination.task ? 'プロジェクト：' + safeMeetingText(destination.task.projectName) : MEETING_PARENT_HINTS[bundle.id] ? 'プロジェクト：未照合' : 'プロジェクト候補：' + (parent?.projects.map(safeMeetingText).join(' / ') ?? '未指定')}</p>
