...
function BriefTask({ task, tasks, now, call, members, names, statusLabel }: { task: DashboardTask; tasks: readonly DashboardTask[]; now: Date; call?: Notification; members: Record<string, TaskViewMember>; names: Record<string, string>; statusLabel?: string }) {
  const dependencies = neoDependencyEvidence(task, tasks);
  return <li>
    <Link href={`/projects/${encodeURIComponent(task.projectId)}/board?task=${encodeURIComponent(task.id)}`} className={`group block ${NEO_TASK_ROW_CLASS}`}>
      <NeoBriefTaskHeading task={task} tasks={tasks} now={now} members={members} names={names} parentTitle={task.parentTitle} statusLabel={statusLabel} />
    </Link>
    ...
    {call && <Link href={notificationTaskHref(call)} className="mt-2 inline-block text-xs text-primary underline">{isNeoAutomationReminder(call) ? '全員分の確認状況（自動確認）' : `${call.senderName || 'メンバー'}からの呼びかけを確認`}</Link>}
  </li>;
}

export function NeoMorningBrief(props: NeoReviewRequestsProps & { taskScope?: BriefTaskScope; assigneeFilterId?: string | null; assigneeFilterName?: string }) {
  const { tasks, userId, isLoading, error, projectTaskStatus, isSample = false, taskScope = 'mine', assigneeFilterId, assigneeFilterName, members = [] } = props;
  ...
  const taskSection = (name: string, items: DashboardTask[], statusLabel?: (task: DashboardTask) => string | undefined, emptyLabel = 'いまはありません。') => <section aria-label={name} className="border-t">
    <div className="flex flex-wrap items-center gap-2 px-5 py-3"><h3 className="inline-flex items-center gap-2 text-sm font-semibold"><Clock3 className="size-4 text-primary" aria-hidden="true" />{name}</h3>{items.length > 0 && <span className="text-xs tabular-nums text-muted-foreground">{items.length}件{incomplete ? '以上' : ''}</span>}</div>
    {items.length ? <ul className={NEO_TASK_LIST_END_CLASS}>{items.map(task => <BriefTask key={neoTaskKey(task)} task={task} tasks={available} now={now} members={memberById} names={names} statusLabel={statusLabel?.(task)} call={brief.taskCalls.find(call => call.projectId === task.projectId && call.taskId === task.id)} />)}</ul> : <p className="px-5 pb-3 text-xs text-muted-foreground">{incomplete ? emptyText : emptyLabel}</p>}
  </section>;
  return <div data-testid="neo-morning-brief">
    <NeoCalendar tasks={[]} isLoading={false} error={null} scheduleOnly />
    <NeoReviewRequests {...props} calls={brief.taskCalls} embedded />
    {(brief.calls.length > 0 || notifications.isLoading || notifications.error) && <section aria-label="返答・確認" className="border-t">
      ...
      {notifications.error ? <p role="alert" className="px-5 pb-3 text-xs text-amber-800">呼びかけの通知を取得できません。</p> : notifications.isLoading ? <p role="status" className="px-5 pb-3 text-xs text-muted-foreground">呼びかけを読み込み中…</p> : <ul>{brief.calls.map(call => {
        ...
        return <li key={call.id}><Link href={notificationTaskHref(call)} className={taskRowInteraction + ' block px-5 py-3'}><p className="break-words text-sm font-medium">{call.taskName || call.title}</p>
    ...
    <p className="border-t px-5 py-3 text-[11px] leading-relaxed text-muted-foreground">{isSample ? '架空データ。' : ''}{scopeDescription}確認依頼・呼びかけ・Google予定は自分宛てのみ。待ちは登録された前提関係を照合しています。{incomplete ? '一部のタスクは未確認です。' : '共有タスクの変更は自動で表示に反映されます。'}</p>
  </div>;
}
