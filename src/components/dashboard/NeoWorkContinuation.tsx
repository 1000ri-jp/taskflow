...
function ActiveWork({ task, tasks, userId }: { task: MyTask; tasks: MyTask[]; userId: string }) {
  ...
  return <section aria-label="選んだ仕事の続き" className="space-y-3 px-5 py-3">
    ...
    <WorkSupportPreparation checklists={detailsReady ? details.checklists : undefined} task={task} tasks={tasks} userId={userId} purpose={scope.project?.description ?? ''} preferenceKey={JSON.stringify([profile, supportOnce.revision])} once={supportOnce.instruction} onConsumed={() => setSupportOnce(value => ({ ...value, instruction: '' }))} disabled={!detailsReady || !canEdit || !support.isSuccess} actions={!context.review && !closed && !task.workState && !blockers.length ? <fieldset disabled={!canEdit || !detailsReady} className="contents"><TaskWorkflow task={task} tasks={tasks} userId={userId} names={names} continuation compact onRecorded={recorded} />{task.workProgress === 'started' && !submitting && <Button size="sm" onClick={() => setSubmissionFor(submissionKey)}><ArrowRight className="h-4 w-4" />担当者へ相談</Button>}</fieldset> : null} />
