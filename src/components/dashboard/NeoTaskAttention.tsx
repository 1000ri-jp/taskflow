...
export function NeoTaskAttention({tasks, userId, isLoading, error, projectTaskStatus}: NeoReviewRequestsProps) {
  ...
  const row = ({parent, children}: typeof issues[number]) => <li key={JSON.stringify([parent.projectId,parent.id])} className="min-w-0 space-y-2">
    <Link prefetch={false} href={href(parent)} className={taskRowInteraction + ' flex min-w-0 flex-wrap items-center gap-x-2 gap-y-1 rounded-md text-sm font-medium'}>
      <ProjectMark name={parent.projectName} color={parent.projectColor} />
  ...
  return <section aria-label="完了状態の確認" className="rounded-2xl border border-amber-300 bg-amber-50 px-5 py-4">
    <div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2"><TriangleAlert className="size-4 text-amber-700" aria-hidden="true" /><h2 className="text-sm font-semibold">完了状態の確認</h2><span className="text-xs text-muted-foreground">{visibleIssues.length}件{partial ? '以上' : ''}</span></div></div>
    ...
    <ul className="mt-3 space-y-4">{visibleIssues.slice(0,5).map(row)}</ul>
    {visibleIssues.length > 5 && <details className="mt-3"><summary className="cursor-pointer text-xs text-primary">ほか{visibleIssues.length - 5}件</summary><ul className="mt-3 space-y-4">{visibleIssues.slice(5).map(row)}</ul></details>}
  </section>;
}
