...
export function TaskCalendarView({ tasks, lists, onTaskClick, onDateChange, onAddTask }: { tasks: Task[]; lists: List[]; onTaskClick: (id: string) => void; onDateChange?: CalendarDateChange; onAddTask?: (listId: string, title: string, dueDate: Date) => void | Promise<void> }) {
  ...
  return <section aria-label="タスクカレンダー" className="space-y-3 p-3">
    ...
    <p className="text-xs text-muted-foreground">実データの開始日・期限を表示します（開催日ではありません）。作業期間の重なりはガントチャートで確認できます。日付をクリックすると、その日を期限にしたタスクを追加できます。</p>
    <div className="overflow-x-auto rounded-lg border bg-background">
      <div className="min-w-[700px]">
        ...
        <div className="grid grid-cols-7">{days.map(date => {
          ...
          return <section key={dateKey} aria-label={format(date, 'yyyy年M月d日')} onClick={onAddTask ? (event) => { if ((event.target as HTMLElement).closest('button, a, input, select, textarea, summary')) return; setNewTaskDate(startOfDay(date)); setNewTaskTitle(''); setIsAddingTask(true); } : undefined} onDragOver={onDateChange ? (event) => { event.preventDefault(); setDragOverDate(dateKey); } : undefined} onDragLeave={onDateChange ? () => setDragOverDate(current => current === dateKey ? null : current) : undefined} onDrop={onDateChange ? (event) => {
          ...
          } : undefined} className={cn('min-w-0 border-r border-b p-2', mode === 'week' ? 'min-h-64' : 'min-h-28', onAddTask && 'cursor-pointer hover:bg-blue-50/40', !isSameMonth(date, anchor) && mode === 'month' && 'bg-muted/30', isSameDay(date, today) && 'bg-blue-50/60', dragOverDate === dateKey && 'bg-blue-100/80 ring-2 ring-inset ring-blue-400')}>
    ...
    {isAddingTask && newTaskDate && onAddTask && <form className="rounded-lg border bg-background p-3" onSubmit={async (event) => { event.preventDefault(); const listId = newTaskListId || lists[0]?.id || ''; if (!listId || !newTaskTitle.trim()) return; await onAddTask(listId, newTaskTitle.trim(), newTaskDate); setIsAddingTask(false); setNewTaskTitle(''); }}><p className="mb-2 text-sm font-medium">{format(newTaskDate, 'yyyy年M月d日')}にタスクを追加（期限）</p><div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_220px_auto_auto]"><Input aria-label="新しいタスクのタイトル" required value={newTaskTitle} onChange={(event) => setNewTaskTitle(event.target.value)} placeholder="タスク名" autoFocus /><select aria-label="追加先のリスト" className="h-9 rounded-md border bg-transparent px-2 text-sm" required value={newTaskListId || lists[0]?.id || ''} onChange={(event) => setNewTaskListId(event.target.value)}><option value="" disabled>リストを選択</option>{lists.map(list => <option key={list.id} value={list.id}>{list.name}</option>)}</select><Button type="submit" disabled={!(newTaskListId || lists[0]?.id) || !newTaskTitle.trim()}>保存</Button><Button type="button" variant="outline" onClick={() => setIsAddingTask(false)}>キャンセル</Button></div></form>}
