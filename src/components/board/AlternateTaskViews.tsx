...
export function AlternateTaskViews({ view, projectId, viewerId, projectMemberIds, tasks, lists, onTaskClick }: {
  view: Exclude<TaskView, 'board'>; projectId: string; viewerId: string; projectMemberIds: string[]; tasks: Task[]; lists: List[]; onTaskClick: (id: string) => void;
}) {
  ...
  return <div className="min-w-0">
    ...
    {view === 'outline' ? <TaskOutlineView projectId={projectId} viewerId={viewerId} tasks={tasks} lists={lists} names={names} members={memberDetails} onTaskClick={onTaskClick} /> : view === 'table' ? <TaskTableView tasks={tasks} lists={lists} names={names} onTaskClick={onTaskClick} /> : <TaskCalendarView tasks={tasks} lists={lists} onTaskClick={onTaskClick} />}
  </div>;
}
