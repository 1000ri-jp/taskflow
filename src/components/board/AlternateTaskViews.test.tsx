...
describe('calendar view', () => {
  ...
  it('opens a dated task form from an empty day and keeps task clicks separate', async () => {
    ...
    fireEvent.click(day);
    expect(screen.getByText('2026年9月8日にタスクを追加（期限）')).toBeInTheDocument();
    await userEvent.setup().type(screen.getByRole('textbox', { name: '新しいタスクのタイトル' }), '追加タスク');
    fireEvent.click(screen.getByRole('button', { name: '保存' }));
    await waitFor(() => expect(add).toHaveBeenCalledWith('list-1', '追加タスク', new Date(2026, 8, 8)));
    fireEvent.click(within(screen.getByRole('region', { name: '2026年9月3日' })).getByRole('button', { name: /期限\s*既存タスク/ }));
    expect(open).toHaveBeenCalledWith(task.id);
  });
});
