...
import { isProjectView, type ProjectView } from '@/lib/board/projectNavigation';
...
function read(): Record<string, ProjectView> {
  ...
  return Object.fromEntries(Object.entries(value).filter(([key, view]) => validScope(key) && isProjectView(view)));
}
interface State {
  byScope: Record<string, ProjectView>;
  hydrated: boolean;
  persistenceFailed: boolean;
  hydrate: () => void;
  setDefault: (scope: string, view: ProjectView) => void;
}
export const useProjectTaskViewStore = create<State>((set, get) => ({
  ...
  setDefault: (scope, view) => {
    if (!validScope(scope) || !isProjectView(view)) return;
