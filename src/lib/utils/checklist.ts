...
export function moveChecklistItem(items: readonly ChecklistItem[], itemId: string, targetId: string) {
  // Completion grouping is display-only. Keep the other group's manual order
  // intact, including where items return if they are checked/unchecked later.
  ...
  if (sorted[from].isChecked !== sorted[to].isChecked) {
    throw new Error('完了状態が変更されています。同じ状態の項目の中で並べ替えてください。');
  }
